        ...
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (array('export') as $view) {
                ...
                foreach ($entityConfig[$view]['fields'] as $fieldName => $fieldMetadata) {
                    ...
                    } elseif (file_exists('../templates/admin/Exporter/field_' . $fieldMetadata['dataType'] . '.html.twig')) {
                        $template = 'admin/Exporter/field_' . $fieldMetadata['dataType'] . '.html.twig';
                    } elseif (in_array($fieldMetadata['dataType'], array('string', 'text', 'FOS\CKEditorBundle\Form\Type\CKEditorType'))) {
                        $template = 'admin/Exporter/field_raw.html.twig';
                    } elseif (array_key_exists($fieldTemplateName, $entityConfig['templates'])) {
                        $template = $entityConfig['templates'][$fieldTemplateName];
                    } else {
                        $template = 'admin/Exporter/label_null.html.twig';
                    }
